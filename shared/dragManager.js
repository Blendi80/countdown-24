export class DragManager {
  ...
  update(mouseX, mouseY, mouseIsPressed) {
    for (const obj of this.dragObjects) {
      let overlapping;
      if (obj.isOverlapping) overlapping = obj.isOverlapping(mouseX, mouseY);
      else
        overlapping =
          dist(obj.target.positionX, obj.target.positionY, mouseX, mouseY) <
          (obj.target.radius || 30);
    ...
    if (this.currentDragObject) {
      this.currentDragObject.target.positionX =
        mouseX + this.currentDragObject.grabOffsetX;
      this.currentDragObject.target.positionY =
        mouseY + this.currentDragObject.grabOffsetY;
    }
  }
}
